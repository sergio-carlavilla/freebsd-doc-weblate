...
El objetivo de este documento es familiarizar rápidamente a los usuarios intermedios y avanzados de Linux(TM) con los conceptos básicos de FreeBSD.
...
Este documento destaca algunas de las diferencias técnicas entre FreeBSD y Linux(TM) para que los usuarios intermedios y avanzados de Linux(TM) puedan familiarizarse rápidamente con los conceptos básicos de FreeBSD.

Este documento asume que FreeBSD ya está instalado. Consulte el capítulo link:{handbook}#bsdinstall[Instalación de FreeBSD] del Manual de FreeBSD para obtener ayuda con el proceso de instalación.

[[shells]]
== Shell por defecto

Los usuarios de Linux(TM) a menudo se sorprenden al descubrir que Bash no es la shell por defecto en FreeBSD. De hecho, Bash no está incluido en la instalación predeterminada. En su lugar, FreeBSD utiliza man:tcsh[1] como shell predeterminada para el usuario root y man:sh[1] como shell compatible con Bourne shell por defecto. man:sh[1] es muy similar a Bash pero con un conjunto de características mucho más pequeño. Generalmente, los scripts escritos para man:sh[1] se ejecutarán en Bash, pero al contrario no siempre es así.
...
[[software]]
== Paquetes y Puertos: Instalar software en FreeBSD
...
Los paquetes son aplicaciones precompiladas, el equivalente de FreeBSD a los ficheros [.filename]#.deb# en los sistemas basados en Debian/Ubuntu y a los ficheros [.filename]#.rpm# en los sistemas basados en Red Hat/Fedora. Los paquetes se instalan usando `pkg`. Por ejemplo, el siguiente comando instala Apache 2.4:
...
En lugar de un identificador genérico _ethX_ como Linux(TM) utiliza para identificar una interfaz de red, FreeBSD usa el nombre del controlador seguido de un número. La salida de man:ifconfig[8] muestra dos interfaces de red Intel(TM) Pro 1000 ([.filename]#em0# y [.filename]#em1#):
...
FreeBSD no usa las IPTABLES de Linux(TM) para su firewall. En su lugar, FreeBSD ofrece tres firewalls a nivel del kernel:
...
Las actualizaciones de los binarios son similares a usar `yum` o `apt-get` para actualizar un sistema Linux(TM). En FreeBSD, man:freebsd-update[8] puede usarse para obtener las nuevas actualizaciones de los binarios e instalarlas. Estas actualizaciones pueden ser programadas usando man:cron[8].
...
En algunas distribuciones de Linux(TM), puede consultar [.filename]#/proc/sys/net/ipv4/ip_forward# para determinar si IP forwarding está habilitado. En FreeBSD, man:sysctl[8] se usa para ver esta y otras configuraciones del sistema.
...
Este documento ha proporcionado una visión general de FreeBSD. Consulte el link:{handbook}[Manual de FreeBSD] para obtener una información más detallada de los temas tratados, así como otros muchos temas que no se han tratado en este documento.
