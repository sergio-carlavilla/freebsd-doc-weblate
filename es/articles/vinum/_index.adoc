...
RAID ofrece varias formas de tolerancia a fallos, aunque RAID-0 es algo engañoso ya que no provee redundancia. El segmentado requiere algo más de esfuerzo para localizar el dato, y puede ocasionar carga de E/S adicional cuando una transferencia está repartida por múltiples discos, pero también puede proporcionar una carga más constante entre los discos. <<vinum-striped>> ilustra la secuencia en la que las unidades de discos están asignadas en una organización segmentada.
...
* El objeto más visible es el disco virtual, llamado _volumen_. Volúmenes tienen esencialmente las mismas propiedades que una unidad de disco UNIX(TM) aunque hay algunas pequeñas diferencias. Una de ellas, que no tienen limitación de tamaño.
...
<<vinum-comparison>> resume las ventajas y desventajas de cada organización plex.

[[vinum-comparison]]
.Organizaciones Plex en [.filename]#vinum#
...
* La línea _plex_ define un lex. El único parámetro requerido es la organización, en este caso _concat_. El nombre no es necesario ya que el sistema genera uno automáticamente a partir del nombre del volumen añadiendo el sufijo _.px_, donde _x_ es el número del plex en el volumen. Por lo tanto este plex se llamará _myvol.p0_.
* La línea _sd_ describe un subdisco. La especificación mínima incluye el nombre de la unidad en la que almacenarlo, y la longitud del subdisco. El bombre no es necesario ya que el sistema asigna un nombre automáticamente derivado del nombre del plex añadiendo el sufijo _.sx_, donde _x_ es el número del subdisco en el plex. Por lo tanto [.filename]#vinum# asigna a este subdisco el nombre _myvol.p0.s0_.
...
Esta salida muestra el formato de lista abreviada de man:gvinum[8]. Está representado gráficamente en <<vinum-simple-vol>>.

[[vinum-simple-vol]]
.Un Volumen [.filename]#vinum# Sencillo
...
<<vinum-mirrored-vol>> muestra la estructura gráficamente.
...
Este volumen está representado en <<vinum-striped-vol>>. El color oscuro de los segmentos indican la posición dentro del espacio de direcciones del plex, donde los segmentos más claros aparecen primero y los más oscuros últimos.

=== Resiliencia y Rendimiento

[[vinum-resilience]]Con suficiente hardware, es posible construir volúmenes que muestren resiliencia aumentada y rendimiento aumentado comparado con particiones UNIX(TM) estándar. Una configuración típica podría ser:
...
<<vinum-raid10-vol>> representa la estructura de este volumen.
...
[.filename]#vinum# asigna nombres por defecto a los plexes y los subdiscos, aunque pueden ser modificados. Modificar los nombres por defecto no está recomendado ya que no proprorciona una ventaja significativa y puede provocar confusión.
...
En el sistema los volúmenes parecen idénticos a los discos, con una excepción. A diferencia de las unidades UNIX(TM), [.filename]#vinum# no particiona los volúmenes, por lo que estos no tienen tabla de particiones. Esto ha requerido modificaciones en algunas utilidades de disco, notablemente de man:newfs[8], de forma que no intente interpretar la última letra de un nombre de volumen [.filename]#vinum#. Por ejemplo, una unidad de disco podría tener un nombre como [.filename]#/dev/ad0a# o [.filename]#/dev/da2h#. Estos nombres representan la primera partición ([.filename]#a#) en el primer (0) disco IDE ([.filename]#ad#) y la octava partición ([.filename]#h#) en el tercer (2) disco SCSI ([.filename]#da#) respectivamente. Por el contrario, un volumen [.filename]#vinum# podría llamarse [.filename]#/dev/gvinum/concat#, que no guarda relación con un nombre de partición.
...
Los valores en los que fijarse son `135680` para el offset, en relación a la partición [.filename]#/dev/da0h#. Esto se traduce en 256 bloques de disco de 512 bytes en términos de `bsdlabel`. De igual modo, el tamaño de este volumen raíz es 245760 bloques de 512 bytes. [.filename]#/dev/da1h#, al contener la segunda réplica de este volumen raíz, tiene una configuración asimétrica.
...
Se puede observar que el parámetro `size` para la partición falsa `a` coincide con el valor esbozado arriba, mientras que el parámetro `offset` es la suma del ofset dentro de la partición [.filename]#vinum#`h`, y el offset de esta partición dentro del dispositivo o rebanada. Esta es una configuración típica que es necesaria para evitar el problema descrito en <<vinum-root-panic>>. La partición `a` entera está completamente dentro de la partición `h` que contiene todos los datos [.filename]#vinum# para este dispositivo.
...
Si por algún motivo el sistema con continúa con el arranque, el proceso de arranque se puede interrumpir presionandokbd:[espacio] durante el aviso de 10 segundos. La variable del cargador `vinum.autostart` puede examinarse tecleando `show` y manipularse usando `set` o `unset`.
...
Cuando está listo, el proceso de arranque puede continuar tecleando `boot -as -as` solicita al kernel que pregunte por el sistema de ficheros raíz a montar (`-a`) y hacer que el proceso de arranque pare en modo de usuario único (`-s`), donde el sistema de ficheros raíz está montado como solo lectura. De ese modo, incluso si sólo ha sido montado un plex de un volumen multi-plex, no hay riesgo de inconsistencia de datos entre plexes.
...
==== Sólo Carga la Configuración de Arranque Primaria

Si [.filename]#/boot/loader# falla al cargar, pero la configuración de arranque primaria todavía carga (visible mediante un sólo guión en la columna de la izquierda de la pantalla justo después de que comience el proceso de arranque), se puede intentar interrumpir el arranque primario presionando kbd:[space]. Esto hará que el proceso de arranque se pare en link:{handbook}#boot-boot1[stage two]. Aquí se puede intentar arrancar desde una partición alternativa, como la partición que contiene sl sistema de ficheros raíz anterior que ha sido movido desde `a`.
