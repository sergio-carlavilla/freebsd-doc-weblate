...
. Construye el árbol fuente (o sólo una parte de él) con advertencias adicionales habilitadas y limpia las advertencias. También se puede encontrar una lista de advertencias de construcción en nuestro https://ci.freebsd.org[CI] seleccionando una construcción y marcando "LLVM/Clang Warnings".
...
* Hay una gran cantidad de ports que no tienen un maintainer. Conviertase en un maintainer y <<adopt-port,adopte un port>>.
* Si ha creado o adoptado un port tenga en cuenta lo <<maintain-port,que debe hacer como maintainer>>.
* Si busca un desafío rápido, puede <<fix-broken,corregir un error o un port roto>>.
...
La Fundación FreeBSD también puede aceptar https://www.freebsdfoundation.org/donate/[donaciones en línea] a través de varias opciones de pago.
...
Primero, asegúrese de de comprender cuáles serán sus <<maintain-port,responsabilidades como maintainer>>. Lea también el link:{porters-handbook}[Handbook del Porter]. _Por favor, no se comprometa con más de lo que crea que puede manejar cómodamente._
...
** Asegúrese de que las dependencias de su port estén completas. La forma recomendada de hacerlo es instalando su propio tinderbox de ports. Tiene más información en la sección de <<resources,recursos>>.
...
** Compruebe su port utilizando man:portlint[1] como guía. Consulte los <<resources,recursos>> para obtener información importante sobre el uso de portlint.
...
. Enviar los cambios
+ 
Envíe su actualización enviando un PR con una explicación de los cambios y un parque que contenga las diferencias entre el port original y el actualizado. Consulte el artículo link:https://www.FreeBSD.org/doc/es_ES.ISO8859-1/articles/problem-reports[Cómo enviar informes de problemas de FreeBSD] para obtener más información sobre como escribir un PR correctamente.
+
[NOTE]
====
...
package:ports-mgmt/poudriere[] es la forma más completa de probar un port a través de todo el ciclo de instalación, empaquetado y desinstalación. La documentación se encuentra en el repositorio de https://github.com/freebsd/poudriere[poudriere en github].

man:portlint[1] es una aplicación que permite verificar que su port cumple con muchas pautas importantes de estilo y funcionamiento. portlint es una aplicación heurística simple, por lo que _solo debe utilizarla como guía_. Si portlint sugiere cambios que parecen no tener sentido, consulte el link:{porters-handbook}[Handbook del Porter] o pida consejo.
